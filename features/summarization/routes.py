...
from flask import (
    Blueprint, render_template, request, flash, current_app, url_for, g, redirect, send_file, jsonify, after_this_request
)
import os
import io
import uuid
...
MAX_CLASSIFICATION_EXCERPT_TEXT_SUMMARY = 15000
...
def classify_and_summarize_with_gemini(text_content, model_name_from_config, filename=""):
    if not text_content or not text_content.strip():
        return "N/A", "No text provided for summarization.", True
...
def process_create_ppt():
    ...
    try:
        ...
        download_url_for_frontend = url_for('summarization.download_generated_ppt', file_id=g.request_id, filename=download_filename)
