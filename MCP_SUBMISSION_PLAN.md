...
- [ ] Write Test Account Setup Instructions — a single-page walkthrough a reviewer who has never seen Synzo can follow:
  1. How to add the custom connector in claude.ai (URL: `https://www.synzo.ai/mcp`).
  2. How to sign in (OAuth via WorkOS AuthKit; credentials from the alias signup).
  3. One prompt per tool, paired with the matching sample file from the bundle. Each prompt burns ~1 call so the full sweep stays well under the 50-call free-tier cap.
  4. Brief note on what to expect from each tool's output (e.g. "structured Markdown summary," "PII replaced with block characters," "PNG with faces blurred").
...
- [ ] Zip the 5 files and host at `https://www.synzo.ai/reviewer-bundle.zip` (Flask static dir route; the bundle is small enough to ship via static — total ~3.3 MB).
