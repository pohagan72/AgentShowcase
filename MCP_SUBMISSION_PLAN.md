...
> **Phase 3 progress as of 2026-06-06:** 8 commits shipped + the six policy-compliance audits completed (all PASS). The five public pages (`/docs`, `/privacy`, `/terms`, `/support`, `/security`) are live and styled to match the app's design system; the tools-table renderer + drift guardrail are wired; the privacy policy was code-reviewed against the actual data-handling paths and four inaccuracies corrected. Remaining: technical hardening (magic bytes, tenacity, SECURITY.md file in repo), MCP Inspector + integration + OAuth tests, mailbox aliases via Cloudflare, README audit, and two small follow-ups surfaced by the audit pass (per-org RPM cap in the `-32003` message; decision on `analyze_image` `fields` selector).
...
**Test-suite gaps remaining (from 2026-06-06 adversarial review of the suite)** — none are reviewer-blockers, listed in rough severity order:
- [ ] `/api/v1/summarize` end-to-end happy/failure paths (currently only auth-shape negatives are tested; the 402/413/429/timeout pipeline is proven against the synthetic `/_test/auth_probe` route, not the real production endpoint). ~5 tests in a new `tests/test_api_summarize_e2e.py`.
- [ ] Timeout pipeline on HTTP path (504 + JSON body + refund is only proven on the MCP path). 1 test in `tests/test_auth_failures.py`.
- [ ] `_resolve_api_key` raise sites not fully exercised: malformed key, revoked-key-on-auth-path, orphan-key (org gone). 3 tests in `tests/test_auth_failures.py`.
- [ ] Tools/call >50MB body cap + 10MB decoded-content cap end-to-end at JSON-RPC layer. 2 tests in `tests/test_mcp_server.py`.
- [ ] `/.well-known/oauth-protected-resource` CORS + SYNZO_PUBLIC_URL fallback. 2 tests in `tests/test_mcp_server.py`.
- [ ] JSON-RPC `id` echo on error responses (only asserted on happy paths today). 1 test.
- [ ] Generic Exception inside handler → `isError=true` + refund (only `ToolError` path is pinned). 1 test.
- [ ] `Allow: POST, OPTIONS` header on 405 response. 0 new tests, just an assertion add.
- [ ] **Policy 5.A leak in [tests/test_api_auth.py:27](tests/test_api_auth.py#L27)** — current assertion accepts `status_code in (401, 500)`. The 500 path lives at [auth.py:94](auth.py#L94) when `WORKOS_JWKS_URL` is unset. Decide whether to (a) fix `auth.py:94` to raise 401 (then tighten the test to 401-only) or (b) document that 500 is the correct response for an operator-config failure. Recommendation: (b) — operator config errors SHOULD be 500 so they're visible in logs; the test should split into two cases (401 for bad token, 500 for unset env var).
...
- [ ] **Author `SECURITY.md`** scoped to the MCP server (existing `SAST_REPORT.md` is for the Flask app). Include `security@synzo.ai` per Anthropic Software Directory Terms ("implement and maintain a mechanism for receiving reports of security vulnerabilities"). The public `/security` route already exists and references the mailbox; this task is the in-repo Markdown file (so contributors discover the disclosure process from the repo too).
...
4. **State as of 2026-06-06 (8 Phase 3 commits in):** Phase 0, 1, 1.5, 2, 2.5.A are done and live. **Gate (d) is fully closed** — API-key auth path proven via [scripts/sweep_tools.py](scripts/sweep_tools.py) on 2026-06-05 (5/5 tools SUCCESS), OAuth path proven via claude.ai on 2026-06-06. Three OAuth fixes shipped on 2026-06-06 (commits `42de97e` + `49915d0`, plus WorkOS dashboard config change) — see §6.2 for the detailed lessons; if reproducing OAuth setup elsewhere, run the four curl checks in §6.2 in order before touching code. `transcribe_audio` dropped from scope; the 5 shipped tools are the submission set. **Phase 3 progress on 2026-06-06 (8 commits, 107/107 tests):** all five public pages — `/docs`, `/privacy`, `/terms`, `/support`, `/security` — built, styled to match the app design system (`.feature-container` + `.feature-grid` + `.message-item.category-info` reused; see [docs_routes.py](docs_routes.py), [templates/partials/](templates/partials/), and the `.legal-section`/`.page-hero` helpers in [static/css/style.css](static/css/style.css)); tools-table renderer + YAML sidecar + startup-fail guardrail wired in [docs_renderer.py](docs_renderer.py); `/about` Legal Notice promoted to canonical `/terms`; global footer in [layout.html](templates/layout.html); privacy policy code-reviewed and four inaccuracies corrected (commit `16a4b30`). Commit chain: `99d483d` → `ffdb73d` → `abb1faa` → `16a4b30` → `2647ac9` → `c844753`. **Phase 3 remaining (in roughly suggested order):** (a) **technical hardening** — author `SECURITY.md` for the repo (the `/security` web route already exists; this is the in-repo Markdown for contributors); add file-type detection via magic bytes (currently extension-based at four sites named in Phase 3 technical-hardening section); wrap Gemini calls with tenacity for transient-5xx retry. (b) **Cloudflare Email Routing** — set up `support@`, `privacy@`, `security@` aliases pointing at Paul's real inbox (the pages already reference these addresses; until configured, mailto links open but emails bounce). (c) **policy-compliance audits** — **DONE 2026-06-06, 6/6 PASS.** All findings inline in §6 Phase 3 policy-audit section. Two small follow-ups noted there: include per-org RPM cap in the `-32003` "Rate limit exceeded" message ([auth.py:404](auth.py#L404), one-line); decide on optional `fields` selector for `analyze_image` (recommended: skip for v1, document the decision in submission notes). (d) **README audit + pre-submission /docs read** — confirm README is a superset of `/docs` and every tool name/description/example prompt on `/docs` is still accurate. (e) **MCP Inspector live validation** + integration tests per remaining tool (translate / redact_pii / analyze_image / detect_faces at the JSON-RPC layer with mocked downstreams; summarize_document is already covered). OAuth flow tests are DONE — see Phase 3 "Technical hardening" section. (f) **API-ownership framing for Gemini** (§7 open question). (g) operationally validate Phase 2.5.A by running [scripts/concurrency_load_test.py](scripts/concurrency_load_test.py) against live with `--concurrency 32` before any public-launch traffic (consumes 32 quota slots per run). (h) revoke any API keys exposed during testing. **Phase 3.5 (submission package)** can run in parallel with the technical hardening — structured to mirror the submission form's six pages. **Every form answer is open and must be re-derived against the current code at submission time** — no answers from prior unrelated form attempts are assumed valid. Phase 3.5 also includes a concrete reviewer-test-bundle spec (one sample file per tool, walkthrough doc, dedicated WorkOS test account).
