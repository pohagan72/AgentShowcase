...
> **Phase 3 progress as of 2026-06-06:** 11 commits shipped; six policy-compliance audits completed (all PASS); all nine test-suite gaps from the same-day adversarial review closed (140/140 tests, was 107). The five public pages (`/docs`, `/privacy`, `/terms`, `/support`, `/security`) are live and styled to match the app's design system; the tools-table renderer + drift guardrail are wired; the privacy policy was code-reviewed against the actual data-handling paths and four inaccuracies corrected. The per-org RPM cap is now named in the `-32003` message (commit `3a33950`); `analyze_image` `fields` selector decided as **skip for v1** (documented as a submission-note item). Remaining: technical hardening (magic bytes at 4 sites, tenacity around Gemini, SECURITY.md file in repo), MCP Inspector live validation + integration tests for the four non-summarize tools, mailbox aliases via Cloudflare (`support@`, `privacy@`, `security@`), README ↔ /docs audit, and Gemini API-ownership framing for §7.
...
> - **Reviewer mailbox** — `mcp-review@synzo.ai` via Cloudflare Email Routing → Paul's inbox. Rationale: the form suggests `mcp-review@anthropic.com` but Paul can't receive WorkOS verification email on an Anthropic-owned address; using a Synzo-domain alias lets us complete signup, capture the password, and dry-run the flow ourselves before handing credentials to reviewers. The form's reviewer-contact field is a separate question and can still surface `mcp-review@anthropic.com` if asked.
...
- [ ] Cloudflare: add `mcp-review@synzo.ai` alias → Paul's inbox (same setup as the `support@` / `privacy@` / `security@` aliases tracked in Phase 3 Cross-cutting).
- [ ] Sign up at `https://www.synzo.ai` as `mcp-review@synzo.ai`. Confirm the org auto-provisions, free-tier plan attaches, and `/dashboard` renders cleanly. Capture the password for the walkthrough doc.
...
- [x] **Walkthrough page built and verified in-process at `/reviewer-walkthrough`** (2026-06-07, commit pending). Live at `https://www.synzo.ai/reviewer-walkthrough` after deploy. Senior-dev-targeted: no MCP/OAuth handholding, terse, structured around tool blocks that show prompt + expected response shape rather than prose. Sections: At-a-glance (server URL, transport, protocol versions, DCR-augmentation note); Test account (credentials block — password injected at request time from `REVIEWER_PASSWORD` env var, NEVER committed); Connect (claude.ai web + Desktop with the work-account caveat from §6.2 + a `curl` snippet for direct JSON-RPC inspection); End-to-end sweep (one prompt per tool paired with the matching file, Presidio entity list called out for `redact_pii`, MTCNN cold-start cost called out for `detect_faces`); Error envelope table (5 JSON-RPC codes + `isError:true` distinction); Contact. Page is **unlisted** — not in footer/nav, served with `Cache-Control: private, no-store` and `X-Robots-Tag: noindex, nofollow` as defense in depth. Partial at [templates/partials/_reviewer_walkthrough_content.html](templates/partials/_reviewer_walkthrough_content.html), route in [docs_routes.py](docs_routes.py).
- [ ] **Set `REVIEWER_PASSWORD` env var in Railway** before announcing the URL to reviewers — until set, the credentials block renders a `<set at submission time>` placeholder so an early leak of the URL doesn't leak the password.
