...
> **Status as of 2026-06-05:** Phase 0, Phase 1, Phase 1.5, Phase 2, Phase 2.5.A complete and live. Waitress runs 32 threads (`WAITRESS_THREADS` env-tunable in [run.py](run.py)); every metered tool invocation runs through a 60s wall-clock timeout in `auth.run_metered_tool()` that refunds the quota slot, meters as `refunded`/`timeout`, and surfaces as JSON-RPC `-32005` (or HTTP 504). All five tools (`summarize_document`, `translate_document`, `redact_pii`, `analyze_image`, `detect_faces`) are live at `https://www.synzo.ai/mcp` (Phase 2.5.A auto-deployed on commit `214e5f3`). 87/87 tests green locally. **`transcribe_audio` is dropped from submission scope** — `features/transcription/routes.py` remains a stub but the MCP tool will not ship; the submission lists 5 tools. **Gate (d) — API-key auth path: PROVEN live.** `scripts/sweep_tools.py` ran against `https://www.synzo.ai/mcp` 2026-06-05: 5/5 tools returned SUCCESS with correct response schemas. Latencies: summarize 3.2s, translate 0.7s, redact_pii 0.4s, analyze_image 1.2s, detect_faces 17.6s (MTCNN cold start — well inside the 60s timeout). Total quota burned: 5 units. **Gate (d) — OAuth path: PENDING.** Inspector v0.22.0 can't drive DCR; needs Claude Desktop validation as the next step. **Next gates: Claude Desktop OAuth test → Phase 3 (docs/SECURITY/magic bytes/Gemini retry) + Phase 3.5 (submission package) in parallel — see §6.**
>
...
- [ ] Live OAuth path validation via Claude Desktop (exercises `_resolve_oauth` + WorkOS DCR + `.well-known/oauth-protected-resource`). Gate to Phase 3.
...
4. As of 2026-06-05: Phase 0, Phase 1, Phase 1.5, Phase 2, Phase 2.5.A are done and live (commit `214e5f3`: Waitress threads=32 + per-tool 60s timeout in `auth.run_metered_tool()` deployed; 87/87 tests green locally). `transcribe_audio` is dropped from submission scope — the 5 shipped tools are the submission set. **API-key auth path proven live** via [scripts/sweep_tools.py](scripts/sweep_tools.py) on 2026-06-05 (5/5 tools SUCCESS). Next concrete actions, in order: (a) **Claude Desktop OAuth validation** — connect to `https://www.synzo.ai/mcp`, complete WorkOS sign-in (DCR + PKCE), verify all 5 tools appear and one runs end-to-end in a chat; this exercises `_resolve_oauth` and is the only remaining gate (d) item; (b) issue a dev API key + run [scripts/concurrency_load_test.py](scripts/concurrency_load_test.py) against `https://www.synzo.ai/mcp` with `--concurrency 32` to operationally validate Phase 2.5.A (note: consumes 32 quota slots per run); (c) once (a) is green, start Phase 3 (docs, SECURITY.md, file-type detection by magic bytes, Gemini retry/circuit-breaker) and Phase 3.5 (submission package) in parallel.
