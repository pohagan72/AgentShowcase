...
- [ ] Add WorkOS signup/login flow + basic dashboard (org settings, API key issue/revoke). Capture `WORKOS_ISSUER` from the first real AuthKit token here. **Path A (multi-tenant) — see §8 for decision rationale.** Concrete tasks:
  - WorkOS dashboard: at `dashboard.workos.com/.../authentication/edit-jwt-template`, add `"org_id": "{{ organization.id }}"` to the JWT template so the `_resolve_oauth` lookup at [auth.py:113](auth.py#L113) finds a value.
  - WorkOS dashboard: confirm redirect URIs (`http://localhost:5001/auth/callback` + `https://www.synzo.ai/auth/callback`) and sign-out redirects are set on the Application — already done 2026-06-04.
  - Flask: new `auth_routes.py` blueprint with `/auth/login`, `/auth/callback`, `/auth/logout`, `/dashboard` (+ `/dashboard/keys/issue` and `/dashboard/keys/<id>/revoke`). **Session-cookie-protected, not `@require_auth`-protected** — see the "two URL surfaces" rule above.
  - `/auth/callback`: on first sign-in for a brand-new WorkOS user, call `workos.organizations.create_organization()` with a default name (e.g. `"{user.first_name}'s Workspace"`), add the user as a member via `workos.user_management.create_organization_membership()`, then re-issue/refresh the AuthKit session so the new token carries the `org_id` claim. Mirror the WorkOS org into our `orgs` table (`workos_org_id` populated) and provision a free-tier `quotas` row for the current period.
  - `/dashboard`: read the WorkOS session, show plan + current quota usage + API key list (show raw key once on issuance, then prefix-only). Issue/revoke buttons.
  - Tests: mock the `workos` SDK so tests stay offline; cover happy-path callback (new user → org created, quota row created, session set) and the dashboard endpoints.
  - One-time capture: have `/auth/callback` log the decoded JWT's `iss` claim on the first successful sign-in; paste the value into local `.env` and Railway service variables as `WORKOS_ISSUER`. Remove the log line after capture.
...
- **Multi-tenancy: every WorkOS user belongs to a WorkOS-managed organization, and our `orgs` table mirrors WorkOS organizations 1:1.** During Phase 1 signup/login implementation (2026-06-04) we briefly considered a simpler "one user = one personal org, key on JWT `sub`" model (Path B) because the JWT template doesn't include `org_id` by default and we wanted to avoid editing it. We rejected Path B and stayed with the original Path A design because:
  - The plan §2/§3 architecture is already org-scoped — `auth.py` keys on `Org.workos_org_id`, the `orgs` schema exists for this reason, and switching would leave a dead column behind.
  - "One user = one org" can't model real B2B sales (admin issues keys for a 10-seat team, org-level billing, shared usage history) without a future schema migration. The plan's Phase 4 metered-API business assumes the multi-tenant model is already there.
  - The cost difference is small: Path A is ~2 hours more code (org-creation-on-signup logic + a small `org_memberships` table mapping WorkOS user IDs → org IDs with roles) plus a 5-minute JWT template edit. Not worth downgrading the model to save 2 hours.
  - The "API ownership for Gemini" question in §7 leans on Synzo positioning as a real B2B product, not a single-user demo. Multi-tenancy is part of that story.
  
  Implication: the WorkOS JWT template at `dashboard.workos.com/.../authentication/edit-jwt-template` must include `"org_id": "{{ organization.id }}"` (or equivalent). The Flask callback handler creates a default organization for first-time signups via the WorkOS SDK and re-issues the token so it carries the `org_id` claim. AuthKit shows an org picker if a user belongs to multiple orgs.
