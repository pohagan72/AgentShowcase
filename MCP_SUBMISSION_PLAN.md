...
> **Status as of 2026-06-05:** Phase 0, Phase 1, Phase 1.5, Phase 2 complete and verified live. **Phase 2.5.A safety nets are coded + tested locally**, not yet deployed: Waitress is bumped from default 4 → 32 threads (`WAITRESS_THREADS` env-tunable in [run.py](run.py)); every metered tool invocation now runs through a 60s wall-clock timeout in `auth.run_metered_tool()` that refunds the quota slot, meters as `refunded`/`timeout`, and surfaces as JSON-RPC `-32005` (or HTTP 504); the operational concurrency test ([scripts/concurrency_load_test.py](scripts/concurrency_load_test.py)) is runnable against any deployment. All five tools (`summarize_document`, `translate_document`, `redact_pii`, `analyze_image`, `detect_faces`) are live at `https://www.synzo.ai/mcp` (auto-deployed on commit `13ddec8`). 87/87 tests green locally (was 82/82; +5 timeout-pipeline tests). `transcribe_audio` is deferred: `features/transcription/routes.py` is a stub. **Next gates: deploy Phase 2.5.A to Railway + run [scripts/concurrency_load_test.py](scripts/concurrency_load_test.py) live; decide `transcribe_audio`; live MCP Inspector validation — see §6.**
>
...
**Phase 2.5.A — Thread bump + hard timeout (immediate, before any public launch). DONE 2026-06-05 (local; not yet deployed).**
...
- [ ] **Deploy** the threads-bump + timeout to Railway and run [scripts/concurrency_load_test.py](scripts/concurrency_load_test.py) against the live deployment with `--concurrency 32` before any public launch.
...
4. As of 2026-06-05: Phase 0, Phase 1, Phase 1.5, Phase 2 are done and live. **Phase 2.5.A is coded + tested locally** (Waitress threads=32, per-tool 60s timeout in `auth.run_metered_tool()`, 87/87 tests green) but **not yet deployed**. Next concrete actions, in order: (a) commit + push Phase 2.5.A so Railway auto-deploys; (b) issue a dev API key from the dashboard and run [scripts/concurrency_load_test.py](scripts/concurrency_load_test.py) against the live URL with `--concurrency 32` (note: this consumes 32 free-tier quota slots per run; use a fresh org or temporarily issue a starter-tier key); (c) decide on `transcribe_audio` (build real Gemini-audio pipeline via `inline_data` audio/* in Gemini 1.5+ or formally drop from submission scope); (d) live MCP Inspector validation against `https://www.synzo.ai/mcp` — gate to Phase 3; (e) once inspector is green, start Phase 3 (docs, SECURITY.md, file-type detection by magic bytes, Gemini retry/circuit-breaker) and Phase 3.5 (submission package) in parallel.
