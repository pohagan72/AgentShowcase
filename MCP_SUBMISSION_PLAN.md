...
- [ ] Add `auth.py` tests: 402 exhausted quota, 413 oversized, 429 rate-limited, refund-on-handler-exception. (Basic 401 + blueprint-registered coverage already in [tests/test_api_auth.py](tests/test_api_auth.py); 16/16 passing.)
...
4. Resume at the first unchecked item in the roadmap. As of 2026-06-04 the Phase 1 schema is live; next up is `auth.py` — `Principal` dataclass, `require_auth` decorator, WorkOS JWT verification, API key resolution, atomic SQL quota decrement.
