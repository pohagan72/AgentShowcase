...
> - **No code changes needed.** Phase 3.6's tool surface, SSRF fetcher, and blob store all do the right thing — this phase is a hosting + framing change, not a refactor. Test suite stays at 232/232 (no churn).
> - **Deployed + reverified 2026-06-10 afternoon** (commit `0081ed6`). All 5 static URLs return 200 + correct sizes via `curl -sI`. claude.ai end-to-end test with the `analyze_image` URL prompt succeeded — single clean tool call, no base64 stalling, structured result with hex color swatches rendered. The 2-day stalling problem is resolved for URL-prompt sweeps. Screenshot #1 captured; screenshots #2–5 blocked on the Pro plan rate-limit reset at 12:10 PM.
> - **Form-fill cheat sheet built: [SUBMISSION_FORM_FILL.md](SUBMISSION_FORM_FILL.md).** Operational artifact for submission day — every form field gets a literal paste-ready answer (or a `<TODO>` flag), tagline/description/use-cases all refreshed for the 6-tool URL-first state, 3 submission-notes paste-blocks pre-written. Verified against live registry + SSRF guard constants.
...
### 6.2.A — 2026-06-08/09 follow-up: the OAuth-working state was not reproducible

Documenting this here so future-us doesn't re-derive the same conclusion from scratch.

**Symptom.** Vlad Dubrovenski reported on 2026-06-08 morning that Synzo's MCP server hung indefinitely when added as a custom connector in Claude Desktop — `tools/list` succeeded, but the WorkOS sign-in popup never appeared, no `tools/call` was attempted, and no log lines were generated on Synzo's side. Paul reproduced the same behavior in claude.ai web after a clean disconnect+reconnect cycle. Both clients showed the connector as "connected" with all 5 tools listed but never actually authenticated.

**Diagnostic chain.** What we verified at the HTTP / OAuth protocol layer:

1. `POST /mcp tools/call` without auth → HTTP 401 + `WWW-Authenticate: Bearer realm="synzo", resource_metadata="https://www.synzo.ai/.well-known/oauth-protected-resource"` — correct
2. `GET /.well-known/oauth-protected-resource` → `authorization_servers: ["https://www.synzo.ai"]` + correct CORS — as expected for the augmented-metadata design
3. `GET /.well-known/oauth-authorization-server` (our augmented wrapper) → metadata with `registration_endpoint` injected, all required RFC 8414 fields present — as expected
4. `POST https://real-vine-49-staging.authkit.app/oauth2/register` → 201 Created with a real `client_id` back — DCR endpoint healthy
5. WorkOS dashboard config → AuthKit enabled, DCR enabled, redirect URIs include `https://claude.ai/api/mcp/auth_callback` — unchanged since 2026-06-06

Despite all five being green, claude.ai never initiated the OAuth flow. No popup, no `tools/call` attempt, no log entries.

**Hypothesis tested and rejected: RFC 8414 issuer mismatch.** We hypothesized that our augmented `oauth-authorization-server` endpoint violated RFC 8414 §3.3 by returning `issuer=<real-vine-49-staging.authkit.app>` while served at `www.synzo.ai/...`. We also noticed that WorkOS's bare AuthKit metadata at `https://real-vine-49-staging.authkit.app/.well-known/oauth-authorization-server` now natively includes `registration_endpoint` — the very field our wrapper existed to inject — which removed the original justification for the wrapper. Shipped commit `e47cd6b` to delete the wrapper and advertise the AuthKit subdomain directly in `oauth-protected-resource`'s `authorization_servers`. After a clean disconnect+reconnect in claude.ai web, **the sign-in popup still did not appear**. Reverted (commit `a58834a`) to restore the 2026-06-06 augmented-metadata wrapper.

**Hypothesis tested and rejected: claude.ai connector dialog accepts Bearer API key in Advanced Settings.** Vlad suggested switching to Static OAuth. The claude.ai "Add custom connector" dialog has Advanced Settings → OAuth Client ID + OAuth Client Secret fields. Pasted the existing `WORKOS_CLIENT_ID` + `WORKOS_API_KEY` (which are the wrong namespace — backend SDK credentials, not AuthKit OAuth clients). WorkOS AuthKit rejected with a clean `application_not_found` error page. That at least confirmed claude.ai *will* initiate OAuth when given populated credentials — the silent-no-popup behavior is specific to the DCR path.

**Hypothesis explored but not implemented: static pre-registered OAuth client via WorkOS Applications.** WorkOS dashboard → Applications surface lets you create per-app `client_id` + `client_secret` + Redirects + API keys. The architectural model is "products that share users+orgs" rather than "third-party clients calling Synzo", but the credentials it issues are usable for static OAuth flows. To actually wire this into Synzo we'd need to: (a) create an Application named "Anthropic claude.ai" with redirect URI `https://claude.ai/api/mcp/auth_callback`, (b) configure its session/CORS settings, (c) update [auth.py](auth.py) `_resolve_oauth` to accept the new `audience` value (currently pinned to `WORKOS_CLIENT_ID`) — either widen to a set of accepted audiences, or route by issuer claim, (d) re-test claude.ai end-to-end, (e) document credentials for Anthropic reviewers. Estimated ~2-4 hours of focused work. **Declined at submission time.** Tagged for v1.1 if a real customer asks for OAuth.

**Diagnostic logging shipped.** Commit `96dd84e` adds structured `oauth_reject:` / `oauth_ok:` / `auth_identify:` / `tools_call:` / `tools_call_auth_failed:` log lines at every relevant raise site and entry/exit. Every future auth attempt is grep-able in Railway Deploy Logs. This stays in production permanently — invaluable when we revisit OAuth.

**The honest unknown.** We never produced a log line corresponding to Vlad's or Paul's failed `tools/call` from claude.ai/Desktop. The clients didn't make the call. Three remaining hypotheses we did not falsify:

1. **claude.ai's MCP client gates on something we don't advertise.** The 2025-11-25 spec/security update added explicit guidance around OAuth consent state-cookie binding, `__Host-` prefixed cookies, and DCR redirect-uri restrictions (see the Obsidian Security writeup linked in Vlad's 2026-06-08 Teams message). It's plausible — though unverified — that claude.ai now requires some signal (CIMD support, a specific scope, a stricter PKCE method, a particular `token_endpoint_auth_methods_supported` value) we don't expose, and silently abandons the OAuth bootstrap when it's missing. WorkOS's metadata advertises `client_id_metadata_document_supported: true` but we don't host a CIMD document; that's one candidate.
2. **Per-tool-call argument-size issue masking as connector issue.** Paul observed claude.ai's chat sandbox visibly struggling to construct `content_base64` arguments for the `analyze_image` tool (sandbox would "Ran 5 commands", "Get the tiny base64", etc., spending minutes before either succeeding or silently giving up). That's a separate bug class from auth — it's the LLM trying to generate a 4MB string into a function-call argument. Two earlier-day successful tool calls from claude.ai (2026-06-08 ~18:30 UTC, visible in deploy logs with `path=api_key_bearer` — see below) happened under unusual conditions; the failure mode is more frequent under normal use. *Note: the 18:30 entries showed `path=api_key_bearer`, which means somewhere along the way claude.ai's sandbox authenticated with `sk_synzo_...` instead of via OAuth bearer; we never confirmed the provenance of that key in claude.ai's session.*
3. **Claude Desktop / claude.ai session caching is sticky enough that "Disconnect" doesn't actually rerun discovery.** Paul did multiple full disconnect → sign-out → sign-in → reconnect cycles; popup never appeared. Could be a UI bug; could be that Anthropic caches connector state per-account beyond what the Disconnect button clears.

**What this changes operationally.** Submission auth pivots to Static API key as primary (see §544 locked decision and §555 reviewer-instructions block). OAuth code stays in place — the augmented metadata wrapper, `_resolve_oauth`, the `/.well-known/*` routes, the WorkOS env var config. All proven working on 2026-06-06 and recoverable for v1.1. Diagnostic logging stays. The day cost ~8 hours and we have honest answers for the submission form. Net effect: stronger submission, plus permanent observability.
...
> **Phase 3 progress as of 2026-06-07:** 11 commits shipped on 2026-06-06 + 15 additional commits on 2026-06-07 (vision UX fix `11a2ff0`, sample-files matrix + plan lock `4af101f`, reviewer-bundle.zip + walkthrough `6defeb7`, email swap `8b1af35`, `/auth/logout` AuthKit-termination fix `bf4081c`, walkthrough revert + inline-credentials plan `9219b22`, plan sync `17429ae`, `SECURITY.md` `fe5c3c3`, magic-byte verification `400a4c6`, tenacity Gemini retry `4535392`, integration tests for the 4 non-summarize tools `900c82e`, mailbox collapse `1d6bf3f`, audit-pass plan sync `47f4c24`, README ↔ `/docs` audit `7554fdd`, API-ownership framing for Gemini `691b8c4`) — deployed to Railway 2026-06-07 through `4535392`; live sweep [scripts/sweep_tools.py](scripts/sweep_tools.py) returned 5/5 SUCCESS post-deploy. Six policy-compliance audits completed (all PASS); all nine test-suite gaps from the 2026-06-06 adversarial review closed; +2 logout regression tests 2026-06-07 (**186/186 tests green**, was 107 at the start of the review pass; +79 total across the pass). Five public pages (`/docs`, `/privacy`, `/terms`, `/support`, `/security`) live and styled. Per-org RPM cap named in `-32003` (commit `3a33950`); `analyze_image` `fields` selector **skip for v1** (documented as a submission-note item). Remaining: **live MCP Inspector validation**. *(Mailbox decision locked 2026-06-07: all per-purpose aliases collapsed to the single live mailbox `paul@redmapleresearch.ca`; Cloudflare DNS migration dropped from scope. README ↔ /docs audit done 2026-06-07; Gemini API-ownership framing locked 2026-06-07 — paste-blocks for the submission-notes field live in Policy 1.E below.)*

**Technical hardening**
- [x] Validate with MCP Inspector locally + against the deployed Railway URL. **DONE 2026-06-08.** Re-validation pass against current production deployment (post-magic-bytes / tenacity / 4-tool refund-path additions). Automated sweep via [scripts/sweep_tools.py](scripts/sweep_tools.py) returned 5/5 SUCCESS on live `https://www.synzo.ai/mcp` (latencies: summarize 4.28s / translate 0.57s / redact 0.29s / analyze 1.27s / detect_faces 4.39s warm). Interactive UI pass via `npx @modelcontextprotocol/inspector` v0.22.0 confirmed: Streamable HTTP connect, Bearer auth with `sk_synzo_...` key, all 5 tools render in Tools tab with descriptions + schemas + four annotation pills matching the [mcp_tools.py](mcp_tools.py) contracts (✗ Read-only / ✗ Destructive / ✓ Idempotent / ✗ Open-world on `summarize_document`; spot-checked the other 4 tools render their tool-specific fields — `target_language`, `mode`, `blur_strength` — correctly). Note: Windows + corporate TLS-intercepting proxy required `--insecure-skip-verify` on the sweep and Inspector's default `NODE_TLS_REJECT_UNAUTHORIZED=0` for the UI; both are local-only escape hatches, the production endpoint itself uses Railway's valid edge cert.
- [x] Write integration tests (pytest) for each MCP tool with mocked Gemini. **DONE 2026-06-07.** Coverage audit against the handler branches in [mcp_tools.py](mcp_tools.py) (rather than re-doing what [tests/test_mcp_server.py](tests/test_mcp_server.py) already had) surfaced that happy-path + bad-ext + tenancy were already pinned for all 4 tools, and `test_magic_bytes.py` already covered file-type mismatch end-to-end through `/mcp`. The remaining gap was the **runtime-failure refund paths** — the class of failure a reviewer would hit if Gemini/MTCNN/Presidio glitched on a real upload. Added 10 tests in [tests/test_mcp_server.py](tests/test_mcp_server.py): `translate_document` (+3 — missing target_language, GEMINI_CONFIGURED=False refund, non-blocked Gemini failure refund), `redact_pii` (+2 — redactor returns None refund, byte-size-fields contract pin), `analyze_image` (+2 — GEMINI_CONFIGURED=False refund, `analyze_image_with_gemini` returns None refund), `detect_faces` (+3 — unsupported extension, `blur_image_opencv` returns None refund, `blur_strength=1` → 35 light-blur mapping). Every failure-path test asserts both `isError=true` (so the model can recover) AND that a `refunded` `usage_events` row was written (so customers aren't charged for downstream failures). **186/186 tests green** (was 176; +10).
...
**Total suite: 186/186** (was 122/122 at the start of this gap-closing pass → +64 tests across 6 files; new files since: `tests/test_api_summarize_e2e.py`, `tests/test_magic_bytes.py`, `tests/test_gemini_retry.py`; existing `tests/test_mcp_server.py` extended with the 4-tool runtime-failure refund-path coverage).
...
> - Support channel: **static page + mailbox + response SLA** at `synzo.ai/support`, mailbox `paul@redmapleresearch.ca` (the existing maintainer mailbox; chosen over `support@synzo.ai` via Cloudflare Email Routing to keep DNS off the registrar and avoid a forwarding hop. Brand-honest for a sole-maintainer SaaS at this stage; revisit if a team forms).
...
- [x] **Build `templates/privacy.html` with the following sections** (mirroring Harvey's structure, scaled to Synzo's actual surface — Harvey's policy is ~7000 words across 12 sections; Synzo's should be ~2000-3000 words across the same skeleton because we have far fewer data flows): **DONE** — [templates/partials/_privacy_content.html](templates/partials/_privacy_content.html). All 12 sections present. **Code-review pass on 2026-06-06 corrected four inaccuracies** (commit `16a4b30`): the local `users` table stores email + WorkOS IDs only, not name; `usage_events` records `auth_method` (not "OAuth sub"); the HTMX upload surface uses S3-compatible scratch storage; S3-compatible was added to §4 providers. Verified no Sentry/Datadog/PostHog/GA SDKs are present — claim about no third-party trackers is grounded in a grep. The skeleton below documents the intended structure section by section:
  ...
  10. **Your Data Protection Rights** — how to exercise: email `paul@redmapleresearch.ca`.
  11. **Updates to this Policy** — versioning + how we notify (email all org owners).
  12. **How to Contact us** — `paul@redmapleresearch.ca` (the single live maintainer mailbox; see the locked-decisions box above for why we collapsed all per-purpose aliases to one address). Optional: postal address — defer unless GDPR explicitly requires (it does for DPO-required orgs; Synzo is below the threshold for mandatory DPO appointment, but providing a contact is best practice).
...
- [x] **Privacy mailbox decision: collapse to `paul@redmapleresearch.ca`.** 2026-06-07: dropped the Cloudflare Email Routing plan for per-purpose aliases (`support@`, `privacy@`, `security@`). All three are now served by the existing maintainer mailbox. Trade-off: reveals the sole-operator shape of Synzo to anyone who reads the address carefully (Red Maple Research, the maintainer entity) instead of fronting it behind a brand alias. Acceptable for a portfolio-stage SaaS at submission. Revisit if a team forms or if a customer ask makes a brand alias necessary.
...
- [x] **Build `templates/support.html` with the following sections:** **DONE** — [templates/partials/_support_content.html](templates/partials/_support_content.html). All six sections present, including the "do not paste document contents" privacy-hygiene warning (uses the existing `.message-item.category-info` component from style.css for consistency). Also a separate `/security` page at [templates/partials/_security_content.html](templates/partials/_security_content.html) for vulnerability disclosure.
  ...
  2. **Contact mailbox**: `paul@redmapleresearch.ca` prominent on the page (decision locked 2026-06-07; see Phase 3 Privacy mailbox decision for the trade-off rationale).
  ...
  6. **Security vulnerabilities**: separate disclosure path — pointed at `/security` (which lists the same `paul@redmapleresearch.ca` mailbox but with the right framing for confidential disclosure, plus the SLA) and at `SECURITY.md` in-repo. Anthropic's Terms require a mechanism for receiving security reports; this is it.
- [x] **Mailboxes resolved 2026-06-07: single live address `paul@redmapleresearch.ca`.** Cloudflare Email Routing dropped from scope; the `/support` and `/security` pages now display the maintainer mailbox directly, no forwarding hop to fail silently. See Phase 3 Privacy mailbox decision for the trade-off rationale and the revisit conditions.
...
- [x] **Audit `README.md` against `templates/docs.html`.** **DONE 2026-06-07** (commit `7554fdd`). README synced: Phase 3 status `~75% → ~95%`, test count `140/140 → 186/186`, Roadmap rewritten to drop 5 stale bullets (SECURITY.md, Cloudflare aliases, magic-byte detection, tenacity retry, per-tool integration tests — all shipped or dropped) and add the real remaining list, error-shape table extended with `-32005 MCP_TIMEOUT`. Drift bug caught on the `/docs` Troubleshooting accordion at the same time: `-32004` had been mislabeled as "Invalid file type" (the live mapping in [mcp_routes.py:63,70](mcp_routes.py#L63-L70) is `MCP_UNITS_EXCEEDED`); fixed by relabeling `-32004` accurately and adding a separate "Invalid file type or bad argument" entry that explains the `isError=true` envelope path so users know their quota gets refunded.
- [x] **Author `SECURITY.md`** scoped to the MCP server (existing `SAST_REPORT.md` is for the Flask app). **DONE 2026-06-07.** [SECURITY.md](SECURITY.md) at repo root mirrors the public `/security` page: scope (MCP / API / dashboard / website), reporting via `paul@redmapleresearch.ca` (the sole live mailbox; see Phase 3 Privacy mailbox decision for why per-purpose aliases were dropped), SLA matches `/security` (3 business days ack, 7 business days assessment), private-repo out-of-band fallback, no-PGP-by-default. Anthropic Software Directory Terms requirement ("implement and maintain a mechanism for receiving reports of security vulnerabilities") satisfied by the combination of `/security` (public) + `SECURITY.md` (in-repo for contributors).
- [x] **Prepare reviewer test credentials + sample documents** — **DONE 2026-06-07.** Reviewer account live (`paul@redmapleresearch.ca`, free tier); all 5 sample files staged + zipped + deployed to `https://www.synzo.ai/static/files/reviewer-bundle.zip`; redact-sample verified end-to-end against the live `/mcp` endpoint via [scripts/verify_redact_sample.py](scripts/verify_redact_sample.py). See Phase 3.5 "Reviewer test bundle" for the full matrix and inline-credentials block.
...
- [x] **Locked 2026-06-07.** Anthropic policy 3.F: "Developers must verify that they own or control any API endpoint, domain, or user interface their Software connects to."

  **The argument in one sentence:** Synzo's MCP server runs on the synzo.ai domain Paul owns and operates; every endpoint a Claude client touches (`/mcp`, `/.well-known/oauth-*`, the OAuth callbacks) is first-party. Gemini is a downstream model provider that Synzo calls server-to-server with its own key, named explicitly in the privacy policy under "Service providers" — analogous to how an SaaS app calls AWS, Stripe, or Twilio.

  **Three concrete properties that back this:**
  1. **Surface ownership.** The user never sees a `generativelanguage.googleapis.com` URL, never authenticates against Google, never has a Google quota deducted. They authenticate to `synzo.ai` via WorkOS AuthKit (OAuth 2.0 + DCR), call tools on `synzo.ai/mcp`, and have their quota deducted from `synzo.orgs[org_id].quotas.calls_remaining` in Synzo's own Postgres.
  2. **Provider transparency.** [/privacy](https://www.synzo.ai/privacy) §4 ("Who we Share Data With") names Google (Gemini), WorkOS, Railway, and Microsoft Presidio with links to each provider's policy. There is no hidden dependency.
  3. **Provider portability.** Gemini is the current model provider, not a structural one. The same five MCP tools route to Gemini today via 3 wrapped non-streaming call sites + 2 streaming sites; swapping to Claude or another model is an internal config change at those 5 sites, not a tool-surface or contract change. The Synzo API is what reviewers and users depend on; the model layer is replaceable.

  **Paste-into-form blocks** (under the form's "About your server" / submission-notes field — exact field name TBD at submission time):

  > **Tagline candidate (≤55 chars):** "Document intelligence for AI agents and apps."
  >
  > **Description candidate (50–100 words):** Synzo is a document-intelligence API for AI agents, applications, and people. Five MCP tools — summarize, translate, redact PII, analyze images, detect faces — read documents and images via base64 input and return structured results. Powered by Google Gemini, with Microsoft Presidio for PII detection and MTCNN for face detection. Multi-tenant from row zero with org-scoped quotas, per-org rate limits, and atomic billing-grade metering. Free tier available; OAuth via WorkOS AuthKit (Dynamic Client Registration supported).
  >
  > **Reviewer notes on API ownership (Policy 3.F):** Synzo's MCP server is first-party — all endpoints (`/mcp`, `/.well-known/oauth-protected-resource`, `/.well-known/oauth-authorization-server`, OAuth callbacks) run on synzo.ai, which Paul O'Hagan (Red Maple Research) owns and operates. Users authenticate to synzo.ai via WorkOS AuthKit, not to any downstream provider. Google Gemini is a downstream model provider called server-to-server with Synzo's own API key (analogous to how an SaaS app uses AWS or Stripe); Gemini is named explicitly in the privacy policy. Other downstream services (Microsoft Presidio for PII detection, MTCNN/OpenCV for face detection) run in-process from Synzo's Python environment. The Synzo tool surface is contract-stable across model providers — swapping to Claude or another model is an internal config change at the wrapped Gemini call sites, not a tool-surface change.

  **If a reviewer pushes back:** the fallback is to add explicit "powered by Google Gemini" disclosure into the public tool descriptions in [mcp_tools.py](mcp_tools.py) (currently behavior-neutral per the Policy 2 prompt-injection audit) and into the homepage listing copy. We don't need that today; the privacy policy disclosure + reviewer notes above should satisfy 3.F as-written.
...
- [x] Sign up at `https://www.synzo.ai` as `paul@redmapleresearch.ca`. Confirm the org auto-provisions, free-tier plan attaches, and `/dashboard` renders cleanly. **DONE** (2026-06-07). Capture the password to paste inline into the submission form's reviewer-instructions field at submission time (held outside the repo per the locked-decisions box; `REVIEWER_PASSWORD` Railway env var was removed when the walkthrough page was reverted).
- [x] Verify the seeded org state in the WorkOS dashboard + Synzo `/dashboard/usage` page (calls_remaining=50, plan='free'). **DONE 2026-06-07** as part of the redact-sample verification (`scripts/verify_redact_sample.py` posts to `/mcp` with the test API key bound to this org; the call would have failed without a live free-tier quota). Re-check at submission time in case any test calls between now and then drain the quota below the 5-call sweep budget; reset with [scripts/seed_dev_org.py](scripts/seed_dev_org.py) if needed.
- [x] **Walkthrough page deleted; credentials + sweep go inline in the submission form** (2026-06-07, commit pending). Earlier 2026-06-07 we built `/reviewer-walkthrough` with `noindex`/`no-store`/unlisted defenses, then deleted it the same day after re-reviewing the threat model: a publicly-reachable URL with the password rendered into the HTML response is security-by-obscurity, full stop. The page existed for one viewer on one day; inline-in-form has zero attack surface and no rotate-on-leak story to maintain. The partial template, the route, and the `REVIEWER_PASSWORD` env-var plumbing were all reverted. Reviewer-bundle download URL (the zip) stays public — the sample files are intentionally non-sensitive.
  ...
  **Paste this verbatim into the Anthropic submission form's reviewer-instructions field** (the form likely accepts markdown; if plain text only, strip the formatting). *This is the authoritative version — refreshed 2026-06-10 for the 6-tool URL-first state. The mirror in [SUBMISSION_FORM_FILL.md](SUBMISSION_FORM_FILL.md) §"Form page 2 — Reviewer test bundle" is kept in sync; both sources should match.*
  ...
  **Tools/Resources/Prompts:** 6 / 0 / 0. Live registry visible at https://www.synzo.ai/docs.
  ...
  6 tools render with full schemas and annotations. We sweep this path on every deploy
  via `scripts/sweep_tools.py` (which exercises the full upload-then-tool chain).
...
### Phase 3.7 — URL-first reviewer flow [DONE 2026-06-10 afternoon]
...
- [x] **Deployed to Railway 2026-06-10 afternoon** (commit `0081ed6`). All 5 sample URLs verified via `curl -sI` post-deploy: each returns 200 OK with correct `Content-Length` (39 KB / 14 KB / 36 KB / 2.9 MB / 324 KB) and a sensible `Content-Type` (image/jpeg for JPGs, application/pdf for the PDF; the .docx files come back as `application/octet-stream` because Flask's `mimetypes` module doesn't know OOXML — harmless because Synzo's tools derive type from filename extension + magic-byte verify, not the upstream Content-Type header).
- [x] **Live-retested in claude.ai 2026-06-10 afternoon** with the `analyze_image` URL prompt — the chat sandbox skipped every base64-construction step (no "Embedding base64 image data", no "Resize image to make base64 smaller", no script-execution side-quests). Single clean `Loaded tools, used Synzo integration` tool call, structured result rendered in seconds with Claude's UI auto-drawing color swatches for the dominant_colors hex codes. The 2-day claude.ai stalling problem is resolved for the URL-prompt case. **This is the moment the URL-first design proved itself end-to-end on the actual chat host.**
- [x] **Screenshot #1 captured** — analyze_image with URL prompt + scene description + extracted_text + dominant_colors palette visible.
- [ ] **Capture screenshots #2–5** for the remaining 4 tools (summarize_document, translate_document, redact_pii, detect_faces) — same URL-prompt pattern, paste from `SUBMISSION_FORM_FILL.md`. Blocked on claude.ai Pro plan session-limit reset (next window opens at 12:10 PM 2026-06-10 per the rate-limit banner shown in the screenshot above).
- [x] **Form-fill cheat sheet built: [SUBMISSION_FORM_FILL.md](SUBMISSION_FORM_FILL.md)** — single-place-of-truth operational artifact for submission day. Field-by-field, organized by form page, with literal answers to paste (or `<TODO>` flags for the open ones). Tagline (46 chars), description (77 words), 3 use-case prompts, 3 reusable submission-notes paste-blocks (URL-first rationale, API ownership per Policy 3.F, multi-tenancy posture), full reviewer-instructions block (refreshed to 6 tools / URL-first / sample-URL list), and per-field checkboxes for the compliance review. Tool inventory cross-checked against live `mcp_tools.TOOLS` registry; SSRF guard constants cross-checked against `url_fetcher.py`.
...
- ~~**API ownership for Gemini.**~~ **Resolved 2026-06-07.** Locked framing lives in §6 Phase 3 "Policy 1.E (intellectual property) — API ownership framing": Synzo is first-party (synzo.ai endpoints, WorkOS AuthKit auth, in-Postgres metering), Gemini is a named downstream model provider (privacy-policy §4 disclosure), and the tool contract is portable across model providers. Three paste-blocks (tagline, description, reviewer-notes) are pre-drafted. If a reviewer pushes back, the fallback is explicit "powered by Google Gemini" disclosure in the tool descriptions + listing copy.
...
- `SECURITY.md` — MCP-server-scoped security disclosures, reporting via `paul@redmapleresearch.ca` (the sole live maintainer mailbox; see Phase 3 Privacy mailbox decision).
...
- ~~Cloudflare Email Routing rules — three new aliases: `support@synzo.ai`, `privacy@synzo.ai`, `security@synzo.ai` → Paul's real inbox.~~ **Dropped 2026-06-07** — all public pages now display `paul@redmapleresearch.ca` directly. No Cloudflare config needed; no forwarding hop to break silently. Revisit if a team forms or a brand alias becomes a customer ask.
...
4. **State as of 2026-06-07 (26 Phase 3 commits in across 2 days):** Phase 0, 1, 1.5, 2, 2.5.A are done and live. **Gate (d) is fully closed** — API-key auth path proven via [scripts/sweep_tools.py](scripts/sweep_tools.py) on 2026-06-05 (5/5 tools SUCCESS), OAuth path proven via claude.ai on 2026-06-06. Three OAuth fixes shipped on 2026-06-06 (commits `42de97e` + `49915d0`, plus WorkOS dashboard config change) — see §6.2 for the detailed lessons; if reproducing OAuth setup elsewhere, run the four curl checks in §6.2 in order before touching code. `transcribe_audio` dropped from scope; the 5 shipped tools are the submission set. **Phase 3 progress as of 2026-06-07 (186/186 tests, was 107 at start of review pass):** all five public pages — `/docs`, `/privacy`, `/terms`, `/support`, `/security` — built, styled to match the app design system (`.feature-container` + `.feature-grid` + `.message-item.category-info` reused; see [docs_routes.py](docs_routes.py), [templates/partials/](templates/partials/), and the `.legal-section`/`.page-hero` helpers in [static/css/style.css](static/css/style.css)); tools-table renderer + YAML sidecar + startup-fail guardrail wired in [docs_renderer.py](docs_renderer.py); `/about` Legal Notice promoted to canonical `/terms`; global footer in [layout.html](templates/layout.html); privacy policy code-reviewed and four inaccuracies corrected (commit `16a4b30`); six policy-compliance audits (all PASS, recorded inline in §6 Phase 3); per-org RPM cap now named in the `-32003` message ([auth.py:404](auth.py#L404), commit `3a33950`); OAuth-path test gap closed (commit `6f48ab4`: 12 resolver tests + 3 OAuth-through-`/mcp` tests pinning the path Anthropic's reviewer hits via claude.ai); all 9 remaining test-suite gaps from the same-day adversarial review closed (commit `f63f00a`: `/api/v1/summarize` e2e, HTTP-path 504, `_resolve_api_key` raise sites, body-size caps at JSON-RPC layer, discovery CORS + URL fallback, JSON-RPC `id` echo on errors, generic-Exception → `isError+refund`, 405+Allow on GET/DELETE, Policy 5.A split). Commit chain: `99d483d` → `ffdb73d` → `abb1faa` → `16a4b30` → `2647ac9` → `c844753` → `796bacc` → `3a33950` → `6f48ab4` → `f63f00a`. **Phase 3 shipped 2026-06-07 (deployed to Railway, live sweep 5/5 SUCCESS post-deploy):** `SECURITY.md` mirrors the `/security` public page for in-repo contributors (commit `fe5c3c3`); magic-byte verification via `filetype` lib wired into all 5 MCP tool sites (commit `400a4c6`); tenacity Gemini retry for transient 5xx errors on the 3 non-streaming Gemini call sites (commit `4535392`); +10 runtime-failure integration tests across the 4 non-summarize tools — refund-on-RuntimeError, Gemini-not-configured refund, redactor/blur-pipeline returning None, byte-size contract pins. **Phase 3 remaining (in roughly suggested order):** (a) **MCP Inspector live validation** — the manual walkthrough against the deployed `/mcp` endpoint; complements the [scripts/sweep_tools.py](scripts/sweep_tools.py) programmatic check by catching UX/protocol-shape issues a script wouldn't notice. (b) operationally validate Phase 2.5.A by running [scripts/concurrency_load_test.py](scripts/concurrency_load_test.py) against live with `--concurrency 32` before any public-launch traffic (consumes 32 quota slots per run). (c) revoke any API keys exposed during testing. *(Done 2026-06-07: README ↔ /docs audit, with a drift bug on `/docs` Troubleshooting `-32004` corrected; Gemini API-ownership framing locked, paste-blocks for the submission-notes field live in §6 Phase 3 "Policy 1.E".)* *(Mailbox / Cloudflare DNS work dropped from scope 2026-06-07: collapsed to the single live `paul@redmapleresearch.ca` across all public pages, SECURITY.md, and the reviewer-form contact field. See Phase 3 Privacy mailbox decision for the trade-off rationale.)* **Phase 3.5 (submission package)** can run in parallel with the technical hardening — structured to mirror the submission form's six pages. **Every form answer is open and must be re-derived against the current code at submission time** — no answers from prior unrelated form attempts are assumed valid. Phase 3.5 also includes a concrete reviewer-test-bundle spec (one sample file per tool, walkthrough doc, dedicated WorkOS test account).
